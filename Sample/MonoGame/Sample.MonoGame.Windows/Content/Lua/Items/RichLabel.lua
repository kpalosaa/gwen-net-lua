...
local function OnOpenWindow(s, e)

	window = Gwen.Window.Create(Gwen.Canvas)
	window.Padding = Gwen.Padding.Three
	window.Title = "RichLabel Window"
	window.DeleteOnClose = true
	window.Size = Gwen.Size.Create(500, 300)
	window.Left = 200 
	window.Top = 100

	scroll = Gwen.ScrollControl.Create(window)
	scroll.Dock = Gwen.Dock.Fill
	scroll.EnableScroll(false, true)
	scroll.AutoHideBars = false

	label = Gwen.RichLabel.Create(scroll)
	label.Dock = Gwen.Dock.Fill

	document = Gwen.Document.Create()

	document.Paragraph(Gwen.Margin.Ten, 20).Text
	(
"In olden times when wishing still helped one, there lived a king whose daughters were all beautiful, but the youngest was so beautiful that the sun itself, which has seen so much, was astonished whenever it shone in her face. Close by the king's castle lay a great dark forest, and under an old lime-tree in the forest was a well, and when the day was very warm, the king's child went out into the forest and sat down by the side of the cool fountain, and when she was bored she took a golden ball, and threw it up on high and caught it, and this ball was her favorite plaything."
	)
	document.Image("gwen.png", Gwen.Size.Create(60, 60), Gwen.Rectangle.Create(32, 32, 448, 448), Gwen.Color.Create("White"), Gwen.Margin.Create(20, 10, 20, 10))
	document.Paragraph(Gwen.Margin.Ten, 20).Text
	(
...
this = Gwen.DockLayout.Create(Gwen.Canvas)
	...
	f1 = Gwen.Font.Create("Arial", 15)
	f2 = Gwen.Font.Create("Times New Roman", 20)
	f3 = Gwen.Font.Create("Courier New", 15, Gwen.FontStyle.Italic)
	f4 = Gwen.Font.Create("Times New Roman", 20, Gwen.FontStyle.Bold .. Gwen.FontStyle.Underline)
	f5 = Gwen.Font.Create("Courier New", 10)

	button = Gwen.Button.Create(this)
	button.Dock = Gwen.Dock.Bottom
	button.Width = 150
	button.Text = "Open a Window"
	button.Clicked.add(OnOpenWindow)

	label = Gwen.RichLabel.Create(this)
	label.Dock = Gwen.Dock.Fill;
	...
	document.Paragraph().
		Font(f1).
			Text("This test uses Arial 15, Red. Padding. ", Gwen.Color.Create("Red")).
		Font(f2).
			Text("This text uses Times New Roman Bold 20, Green. Padding. ", Gwen.Color.Create("Green")).
		Font(f3).
			Text("This text uses Courier New Italic 15, Blue. Padding. ", Gwen.Color.Create("Blue")).
		Font().
			Text("Test link (").
			Link("Test Link 1", "Test Link 1", Gwen.Color.Create("Blue"), Gwen.Color.Create("LightBlue")).
			Text("). ").
		Font(f2).
			Text("Test link custom font (", Gwen.Color.Create(255, 0, 255)).
			Link("Test Link 2", "Test Link 2", Gwen.Color.Create("Blue"), Gwen.Color.Create("LightBlue"), f4).
			Text(").", Gwen.Color.Create(255, 0, 255))
	...
	document.Image("gwen.png", Gwen.Size.Create(100, 100))

	label.Document = document

AddItem("Non-Interactive", "RichLabel", this)
